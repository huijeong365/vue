...
<section>
    ...
    <modal v-if="editModal" @close="editModal=false">
            <h3 slot="header" style="color: #ffaa00;">
                <i class="fas fa-exclamation-triangle" aria-hidden="true"></i>
                내용을 수정합니다.
            </h3>
            <span slot="footer">
                <input type="text" v-model="currentWish.value" class="modal1" readonly /><br/>
                <input type="text" v-model="editWishObj.text" v-on:keypress.enter="editWishAction()"
                        onfocus="this.select()" class="modal2" /><br/>
                <button type="button" @click="editWishAction()">수정</button>
                <button type="button" @click="editModal=false" style="margin-left: 35px;">
                        취소</button>
            </span>
        </modal>
</section>
